      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="员工头像">
            <ImageUpload ref="selfInfo"></ImageUpload>
            <!-- 放置上传图片 -->
          </el-form-item>
        </el-col>
      </el-row>
    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <!-- 放置上传图片 -->
          <ImageUpload ref="basicInfo"></ImageUpload>
        </el-form-item>
  ...
  methods: {
    async getUserDetailById () {
      ...
      if (this.userInfo.staffPhoto && this.userInfo.staffPhoto.trim()) {
        this.$refs.selfInfo.fileList = [{ url: this.userInfo.staffPhoto, upload: true }]
      }
    },
    async getPersonalDetail () {
      this.formData = await getPersonalDetail(this.userId)
      if (this.formData.staffPhoto && this.formData.staffPhoto.trim()) {
        this.$refs.basicInfo.fileList = [{ url: this.formData.staffPhoto, upload: true }]
      }
    },
    async saveUser () {
      const newFileList = this.$refs.selfInfo.fileList
      // 如果图片上传成功了，才允许进行保存
      if (newFileList.some(item => !item.upload)) {
        this.$message.warning('有头像尚未上传完成')
        return
      }
      // 如果头像没有上传为空，也要允许用户上传，用空格代替，请求的时候再进行后处理
      await saveUserDetailById({ ...this.userInfo, staffPhoto: newFileList.length ? newFileList[0].url : ' ' })
      this.$message.success('保存用户基本信息成功')
    },
    async savePersonal () {
      const newFileList = this.$refs.basicInfo.fileList
      // 如果图片上传成功了，才允许进行保存
      if (newFileList.some(item => !item.upload)) {
        this.$message.warning('有头像尚未上传完成')
        return
      }
      await updatePersonal({ ...this.formData, staffPhoto: newFileList.length ? newFileList[0].url : ' ' })
